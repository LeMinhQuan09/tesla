...
if (false === class_exists('WF_Licensing_301')) {
  class WF_Licensing_301
  {
    public $prefix = '';
    ...
    function init()
    {
      if (is_admin()) {
        ...
        wp_enqueue_script('wf_licensing_301', $this->js_folder . 'wf-licensing.js', array(), 1.0, true);
        wp_localize_script('wf_licensing_301', 'wf_licensing_' . $this->prefix, $vars);
      }
    } // init
    ...
    {
      $out = array('license_key' => $license_key, 'error' => '', 'name' => 'Special License', 'last_check' => time(), 'valid_until' => '2035-01-01');
