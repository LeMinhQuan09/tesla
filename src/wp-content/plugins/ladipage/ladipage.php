...
function sendToLadiFlow($data) {
	...
	if ($config['ladiflow_api_key'] && $config['ladiflow_webhook_active'] == 1) {
		if ($data['form_id'] && $ladiflowHookConfig[$data['form_id']] && $ladiflowHookConfig[$data['form_id']]['url']) {
			$url = $ladiflowHookConfig[$data['form_id']]['url'];
		} else {
			$url = @$config['ladiflow_api_url'] . '/customer/create-or-update';
		}
		curlPost($url, $data, $config['ladiflow_api_key']);
	}
}
...
function curlPost($url, $data, $apiKey = '4fb4d9fbd59070b3a5e5706bdc6e80c3cf8678c4083d6ae6') {
...
if ( ! class_exists( 'Ladipage' ) ) {
	...
	class Ladipage {
		...
		public function sniff_requests() {
			...
			$isConnectedLadiFlow = (int) @$_GET['is_connected_ladiflow'];
			...
			if ($isConnectedLadiFlow && $ladiFlowApiKey && $actProduct === 'verify_webhook') {
				...
				$this->connectLadiFlowWebhook($ladiFlowApiKey, $apiKey, $config, $webhookURL, $events, $webhookApiVersion, $webhookApiUserId);
			}
			
			if ($ladiFlowApiKey && $actProduct === 'delete_webhook') {
				$this->deleteLadiFlowWebhook($ladiFlowApiKey, $apiKey, $config);
			}
		...
		public function deleteLadiFlowWebhook($ladiFlowApiKey = null, $apiKey = null, $config) {
			try {	
				if (!$ladiFlowApiKey || !$apiKey) {
					wp_send_json(
							array(
								'code'    => 500,
								'data' => null,
				...
				if ( $apiKey === $config['api_key'] ) {
					...
					if ( class_exists( 'WooCommerce' ) ) {
						...
						foreach($results as $result) {
							if (strpos($result->delivery_url, $config['ladiflow_api_url']) !== false) {
								$wh = new WC_Webhook();
		...
		public function connectLadiFlowWebhook($ladiFlowApiKey = null, $apiKey = null, $config = [], $webhookURL, $events = '') {
			try {	
				global $wpdb;
				if (!$ladiFlowApiKey || !$apiKey || !$webhookURL) {
					wp_send_json(
							array(
								'code'    => 500,
								'data' => null,
				...
				if ( $apiKey === $config['api_key'] ) {
					$config['ladiflow_api_key'] = $ladiFlowApiKey;
						...
						if (count($_events) > 0) {
							...
							for ($i = 0; $i < count($_events); $i++) {
								...
								if ($_events[$i] === 'order.created')
									$name = "LadiFlow Create Order";

								if ($_events[$i] === 'order.updated')
									$name = "LadiFlow Update Order";

								if ($_events[$i] === 'customer.created')
									$name = "LadiFlow Create Customer";

								if ($_events[$i] === 'customer.updated')
									$name = "LadiFlow Update Customer";

								$_webhookURL = $webhookURL . '/webhook/wordpress';
								$webhookID = $this->createWebhook($name, $_events[$i], $config['ladiflow_api_key'], $_webhookURL, 'active');
								if ($webhookID) {
		...
		public function searchProduct($keyword = '', $categoryIds = null, $page = 0, $limit = 50) {
			try {
				...
				wp_send_json( array(
					'code'    => 200,
					'data' => array('products' => $productData, 'total_record' => $count)
				) );
			} catch (Exception $ex) {
				wp_send_json(
					array(
				...
				if ( class_exists( 'WooCommerce' ) ) {
					...
					if ($productData) {
						wp_send_json(
							array(
								'code'    => 200,
								'data' => array('product' => $productData),
								'message' => __( 'Success' )
							)
						);
					} else {
		...
		public function addToCart($productID = 0, $qty, $variantID = 0, $arrOptions = []) {
			try {
									...
									<td>
										<?php foreach ($forms as $form) : ?>
											<div class="hook-item">
												...
												<input id="<?php echo $form->ID;?>" data-form-type="wpcf7_contact_form" data-form-id="<?php echo $form->ID;?>" type="text" value="<?php echo @$ladiflowHookConfigs[$form->ID]['url'];?>" placeholder="Webhook URL"/>
											</div>
										<?php endforeach;?>
									</td>
								</tr>
								<tr>
									<td>
										...
										<div class="hook-item">
											...
											<input id="<?php echo $form->ID;?>" data-form-type="wpforms" data-form-id="<?php echo $form->ID;?>" type="text" value="<?php echo @$ladiflowHookConfigs[$form->ID]['url'];?>" placeholder="Webhook URL"/>
										</div>
										<?php endforeach;?>
									</td>
								</tr>
 ...
                                            data: _data
		...
		public function ladiflow_save_hook() {
			...
			print_r($_POST['data']);
			update_option( 'ladiflow_hook_configs', $_POST['data'] );
			exit;
		}
